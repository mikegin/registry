...
feature -- command
	put_alien(id: INTEGER_64 ; name1: STRING ; dob: TUPLE[d: INTEGER_64; m: INTEGER_64; y: INTEGER_64] ; country: STRING)
    	...
    	do
			...
			elseif country.is_empty or (not country.is_empty and then not country.item (1).is_alpha) then
				model.set_message (err_msg_access.m.err_country_start)
    		else
				model.set_message (err_msg_access.m.no_error)

				create dob1.make (dob.y.as_integer_32, dob.m.as_integer_32, dob.d.as_integer_32)

				model.put_alien (id, name1, dob1, country)
    		end
